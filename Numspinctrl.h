...
//
// This simple class allows you to work with floats for spin control.
//
...
// Operations
public:
	void GetRangeAndDelta(float &lower, float& upper, float& delta);
	void SetRangeAndDelta(float lower, float upper, float delta);
...
protected:
	void InitSpinCtrl();
	void SetIntPos (float pos);
	void SetValueForBuddy (float val);
	CString FormatValue (float val);
